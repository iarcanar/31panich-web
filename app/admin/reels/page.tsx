...
const PROD_ADMIN_URL = "https://31panich.co.th/admin/reels"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3" onClick={closeForm}>
          ...
          >
            ...
            <div className="p-4 space-y-4">
              {isDev && <DevWarningBanner compact />}
              {error && (
...
/** Persistent warning shown on dev (localhost) so admin doesn't accidentally
 *  add reels here thinking they'll appear on production. Dev writes go to
 *  local JSON only — production Redis stays untouched. */
function DevWarningBanner({ compact }: { compact?: boolean }) {
  return (
    <div className={`bg-amber-500/10 border-2 border-dashed border-amber-500/50 rounded-xl ${compact ? "p-3 mb-0" : "p-4 mb-5"}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className={`font-bold text-amber-300 ${compact ? "text-xs" : "text-sm"}`}>
            คุณกำลังใช้ Dev (localhost)
          </div>
          <p className={`text-amber-200/80 mt-0.5 th-text ${compact ? "text-[11px]" : "text-xs"}`}>
            Reel ที่เพิ่มที่นี่ <strong>จะไม่ปรากฏบนเว็บจริง</strong> — data เขียนลง local JSON เท่านั้น
          ...
          <a
            ...
            className={`inline-flex items-center gap-1.5 mt-2 h-8 px-3 rounded-lg bg-amber-500 hover:bg-amber-400 text-black font-semibold transition-colors cursor-pointer whitespace-nowrap ${compact ? "text-[11px]" : "text-xs"}`}
          >
            เปิดหน้า Admin จริง
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
          ...
          <div className={`text-[10px] text-amber-200/50 mt-1.5 font-mono`}>
            {PROD_ADMIN_URL}
          </div>
        </div>
      </div>
